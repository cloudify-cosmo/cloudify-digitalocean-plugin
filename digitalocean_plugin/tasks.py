...
@operation
def create(security, droplet_name=None, region=None, image=None,
           size_slug='512mb', backups=False, **_):
    """
    Tell the API to create a droplet. Note that not all combinations of options
     are possible
    :param droplet_name: formal name
    :param region: region to choose
    :param image: image to use
    :param size_slug: size slug - this value determines RAM, CPU, bandwidth,
    and cost of the Droplet.
    ...
    ctx.logger.debug("Create operation executing with params: droplet_name = "
                     "'{0}', region = '{1}', image = '{2}', size_slug = '{3}',"
                     " backups = '{4}'."
                     .format(droplet_name, region, image, size_slug, backups))
